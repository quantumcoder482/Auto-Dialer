...
const state = {
  ...
  dialerSidebar: false                                   // dialer Sidebar Handler
}
...
const getters = {
	darkMode: state => {
		...
		let time = moment(),
		  	...
		  	afterTime = moment('5:00:00 AM', format).add(1, 'days');
...
const actions = {
  ...
  dialerSidebar(context){
    context.commit('dialerSidebarHandler');
  }
}
...
const mutations = {
  ...
  dialerSidebarHandler(state){
    state.dialerSidebar = !state.dialerSidebar
  }
}
