<template>
   <v-list-tile class="sidebar-profile">
		...
		<v-list-tile-content class="ml-3">
			<v-list-tile-title><span>{{user.details.firstName | capitalize}} {{user.details.lastName | capitalize}}</span></v-list-tile-title>
		</v-list-tile-content>
		<v-menu
			bottom
			offset-y
		...
		>
			...
			<div class="dropdown-content">
				<div class="dropdown-top white--text primary">
					<span class="white--text fs-14 fw-bold d-block">{{user.details.firstName | capitalize}} {{user.details.lastName | capitalize}}</span>
...
export default {
  ...
  methods: {
    logoutUser() {
      this.$store.dispatch("logoutUserFromLoopback", this.$router);
	 },
	 getMenuLink(path) {
